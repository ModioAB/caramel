...
def get_log_level(argument_level, logger=None, env=None):
    ...
    environment and root, capping it to between 10-40, returning the log level"""

    if env is None:
        env = os.environ
    env_level = LOG_LEVEL[env.get("CARAMEL_LOG_LEVEL", "ERROR").upper()]

    if logger is None:
        logger = logging.getLogger()
    current_level = logger.level

    argument_verbosity = 40 - argument_level * 10
